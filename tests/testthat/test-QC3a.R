test_that("QC3a T1", {
             ...
             x <- QC3a(d_metingen = metingen, d_parameter = parameter)
             ...
             ids <- x_attr[["QC3a"]][["oordeel"]][["twijfelachtig"]]
             qcids <- metingen$qcid
             ...
             x1 <- x_attr[["QC3a"]][["resultaat"]]
             expect_equal(length(x1), 3)

             x1 <- x_attr[["QC3a"]][["resultaat"]][["plot_param"]][[1]]
...
test_that("QC3a T2", {
             ...
             x <- QC3a(d_metingen = d2, d_parameter = parameter)
             x_attr <- attr(x, "qcout")
             ids <- x_attr[["QC3a"]][["oordeel"]][["twijfelachtig"]]
             qcids <- d2$qcid
             v1 <- setdiff(ids, qcids)
