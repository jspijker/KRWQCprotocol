test_that("QC4_new_a T1", {
  
  
  data(metingen)
  
  x <- QC4_new_a(d_metingen = metingen)
  
  # test if attributes exist
  expect_true(qcout_attrexists(x))
  x_attr <- attr(x, "qcout")
  expect_false(is.null(x_attr[["QC4_new_a"]]))
  expect_true(is.list(x_attr[["QC4_new_a"]][["resultaat"]]))
  
  
  ids <- x_attr[["QC4_new_a"]][["oordeel"]][["twijfelachtig"]]
  qcids <- metingen$qcid
...
test_that("QC4_new_a T3", {
  ...
  x <- QC4_new_a(d_metingen = d)
      x_attr <- attr(x, "qcout")
      ids <- x_attr[["QC4_new_a"]][["oordeel"]][["niet uitvoerbaar"]]
