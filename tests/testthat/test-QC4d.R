test_that("QC4d T1", {
              ...
              x <- QC4d(d_metingen = d, ph_veld_naam = "pH_veld")

              # test if attributes exist
              expect_true(qcout_attrexists(x))
              x_attr <- attr(x, "qcout")
              expect_false(is.null(x_attr[["QC4d"]]))
              expect_true(is.list(x_attr[["QC4d"]][["resultaat"]]))

              # test if ids are from metingen data.frame
              ids <- x_attr[["QC4d"]][["oordeel"]][["verdacht"]]
              qcids <- metingen$qcid
...
test_that("QC4d T3, ph_veld default value", {
