...
In QC1c wordt de informatie uit de parameter tabel (d_parameter) vergeleken met
...
In QC1d worden de bemonsteringsapparatuur en -procedure en de analysetechniek en 
...
QC1f (controle conformiteit BRO format) uit het Protocol wordt niet in dit 
...
### QC3 - Plausibiliteit
In QC1b wordt getoetst of de putten in de BRO puttenlijst voorkomen. De bedoeling
is om d.m.v. een API de puttenlijst uit de BRO automatisch in te laden. Door 
technische problemen is dit momenteel nog niet mogelijk. Hierdoor is voor QC1b 
...
In QC4a wordt de ionenbalans opgesteld. Het is hierdoor belangrijk om de benodigde 
...
Met het voorbeeld metingen bestand uit het LMG wordt QC4a op de volgende manier 
uitgevoerd:

```{r eval = FALSE}
x <- QC4a(d_metingen = x, ph_naam = "h_5__veld", hco3_naam = "hco3_veld", verbose = TRUE)

```

In QC4b worden de ec lab en ec veld vergeleken. Benodigde parameternamen zijn 
...
```{r eval = FALSE}
x <- QC4b(d_metingen = x, d_parameter = parameter, geleidendheid_veld_naam = "ec_5__veld", verbose = TRUE)

```

In QC4c wordt de ec berekend aan de hand van de gemeten concentraties. De berekende
...
de EC te berekenen om deze uiteindelijk toe te passen. Net zoals bij QC4a hoeven 
...
In QC4e wordt de pH lab met pH veld vergeleken. Benodigde parameternamen zijn 
...
In QC4f wordt de pH met hco3 vergeleken. Aangezien in de meeste gevallen hco3
...
In QC4g wordt no3 met nh4 vergeleken. Benodigde parameternamen zijn "NO3" en "NH4". 

### QC4 - Consistentie
In QC3a worden de metingen uit de te valideren meetronde gecontroleerd of deze
...
```{r eval = FALSE}
x <- QC3a(d_metingen = x, d_parameter = parameter)

x_attr <- attr(x, "qcout")

ggsave(
  filename = paste0(getwd(), "/parameters_", Sys.Date(),".pdf"),
  plot = gridExtra::marrangeGrob(x_attr$QC3a$resultaat$plot_param, nrow = 1, ncol = 1),
  width = 15, height = 9
)
```

QC4d wordt regulier uitgevoerd met het aanroepen van QC4d() met metingen data uit
...
van het voorbeeld metingen data uit het LMG wordt QC4d op de volgende manier 
uitgevoerd:

```{r eval = FALSE}
x <- QC4d(d_metingen = x, ph_veld_naam = "h_5__veld", verbose = TRUE)
...
(QC0a, QC0c t/m QC0e, QC1h, QC2a t/m QC2c, QC4a t/m QC4g", "QC3a" en "QC4d") zijn doorlopen
