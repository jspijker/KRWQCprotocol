...
```{r eval=FALSE}
remotes::install_github("rivm-syso/KRWQCprotocol")
```
...
In QC1_new_c wordt de informatie uit de parameter tabel (d_parameter) vergeleken met
...
In QC1_new_d worden de bemonsteringsapparatuur en -procedure en de analysetechniek en 
...
QC1_new_f (controle conformiteit BRO format) uit het Protocol wordt niet in dit 
...
### QC3 - Plausibiliteit
In QC1_new_b wordt getoetst of de putten in de BRO puttenlijst voorkomen. De bedoeling
is om d.m.v. een API de puttenlijst uit de BRO automatisch in te laden. Door 
technische problemen is dit momenteel nog niet mogelijk. Hierdoor is voor QC1_new_b 
...
In QC4_new_a wordt de ionenbalans opgesteld. Het is hierdoor belangrijk om de benodigde 
...
Met het voorbeeld metingen bestand uit het LMG wordt QC4_new_a op de volgende manier 
uitgevoerd:

```{r eval = FALSE}
x <- QC4_new_a(d_metingen = x, ph_naam = "h_5__veld", hco3_naam = "hco3_veld", verbose = TRUE)

```

In QC4_new_b worden de ec lab en ec veld vergeleken. Benodigde parameternamen zijn 
...
```{r eval = FALSE}
x <- QC4_new_b(d_metingen = x, d_parameter = parameter, geleidendheid_veld_naam = "ec_5__veld", verbose = TRUE)

```

In QC4_new_c wordt de ec berekend aan de hand van de gemeten concentraties. De berekende
...
de EC te berekenen om deze uiteindelijk toe te passen. Net zoals bij QC4_new_a hoeven 
...
In QC4_new_e wordt de pH lab met pH veld vergeleken. Benodigde parameternamen zijn 
...
In QC4_new_f wordt de pH met hco3 vergeleken. Aangezien in de meeste gevallen hco3
...
In QC4_new_g wordt no3 met nh4 vergeleken. Benodigde parameternamen zijn "NO3" en "NH4". 

### QC4 - Consistentie
In QC3_new_a worden de metingen uit de te valideren meetronde gecontroleerd of deze
...
```{r eval = FALSE}
x <- QC3_new_a(d_metingen = x, d_parameter = parameter)

x_attr <- attr(x, "qcout")

ggsave(
  filename = paste0(getwd(), "/parameters_", Sys.Date(),".pdf"),
  plot = gridExtra::marrangeGrob(x_attr$QC3_new_a$resultaat$plot_param, nrow = 1, ncol = 1),
  width = 15, height = 9
)
```

QC4_new_d wordt regulier uitgevoerd met het aanroepen van QC4_new_d() met metingen data uit
...
van het voorbeeld metingen data uit het LMG wordt QC4_new_d op de volgende manier 
uitgevoerd:

```{r eval = FALSE}
x <- QC4_new_d(d_metingen = x, ph_veld_naam = "h_5__veld", verbose = TRUE)
...
(QC0a, QC0c t/m QC0e, QC1_new_h, QC2a t/m QC2c, QC4_new_a t/m QC4_new_g", "QC3_new_a" en "QC4_new_d") zijn doorlopen
